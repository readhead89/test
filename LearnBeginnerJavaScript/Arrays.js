/*
Создать массив
Один из способов создать массив — использовать литерал массива . Литерал массива создает массив, заключая элементы в квадратные скобки []. Помните из предыдущего упражнения, что массивы могут хранить данные любого типа — у нас может быть массив, содержащий все типы данных, или массив, содержащий разные типы данных.
Диаграмма, изображающая литерал массива, который состоит из 3 отдельных элементов, запятая разделяет каждый элемент (строку, число и логическое значение), а элементы заключены в квадратные скобки.
Давайте подробнее рассмотрим синтаксис в примере с массивом:
Массив представлен квадратными скобками []и содержимым внутри.
Каждый элемент содержимого внутри массива называется элементом .
Внутри массива есть три разных элемента.
Каждый элемент внутри массива представляет собой отдельный тип данных.
Мы также можем сохранить массив в переменную. Вы могли заметить, что мы сделали это в предыдущем упражнении:
let newYearsResolutions = ['Keep a journal', 'Take a falconry class', 'Learn to juggle'];
Давайте потренируемся, создав собственный массив.
Объявите переменную с помощью const named hobbies и установите ее равной массиву с тремя строками внутри.
Используйте console.log()для печати hobbies на консоль.
 */
...
/*
Свойство.length
Свойство .length массива JavaScript указывает количество элементов, содержащихся в массиве.

    константные числа = [ 1 , 2 , 3 , 4 ] ;

номера . длина // 4
Индекс
Элементы массива упорядочены по значениям индекса , начиная с 0индекса первого элемента. Доступ к элементам можно получить по их индексу, используя имя массива и индекс, заключенный в квадратные скобки.

// Доступ к элементу массива
    const myArray = [ 100 , 200 , 300 ] ;

консоль . журнал ( мой массив [ 0 ] ) ; // 100
консоль . журнал ( мой массив [ 1 ] ) ; // 200
консоль . журнал ( мой массив [ 2 ] ) ; // 300
Метод.push()
Метод .push()массивов JavaScript можно использовать для добавления одного или нескольких элементов в конец массива. .push()изменяет исходный массив и возвращает новую длину массива.

// Добавляем один элемент:
    const cart = [ 'яблоко' , 'апельсин' ] ;
тележка . толчок ( "груша" ) ;

// Добавление нескольких элементов:
константные числа = [ 1 , 2 ] ;
номера . нажать ( 3 , 4 , 5 ) ;
Метод.pop()
Метод .pop()удаляет последний элемент из массива и возвращает этот элемент.

    const ингредиенты = [ 'яйца' , 'мука' , 'шоколад' ] ;

const poppedIngredient = ингредиенты . поп ( ) ; // 'шоколад'
консоль . бревно ( ингредиенты ) ; // ['яйца', 'мука']
Изменяемый
Массивы JavaScript являются изменяемыми , что означает, что содержащиеся в них значения могут быть изменены.

    Даже если они объявлены с помощью const, содержимым можно манипулировать, переназначая внутренние значения или используя такие методы, как .push()и .pop().

    константные имена = [ 'Алиса' , 'Боб' ] ;

имена . толчок ( "Карл" ) ;
// ['Алиса', 'Боб', 'Карл']
Массивы
Массивы — это списки упорядоченных, сохраненных данных. Они могут содержать элементы любого типа данных. Массивы создаются с помощью квадратных скобок с отдельными элементами, разделенными запятыми.
